...
local validate_ipgid = gen_validate_num(0, 63)
local validate_ipgids = gen_validate_str(1, 256)
local validate_ranges_str = gen_validate_str(2, 64)
local validate_desc = gen_validate_str(1, 64, true)
...
function cmd_map.ipgroup_get()
    ...
    local r, e = mysql_select(sql,  1000)
    return r and reply(r) or reply_e(e)
end
log.setlevel("1,2,3,4,d,e,i,f")
...
function cmd_map.ipgroup_set()
    local m, e = validate_post({
        ipgid = validate_ipgid,
        ipgrpname = validate_name,
        ipgrpdesc = validate_desc,
        ranges = validate_ranges_str
    })
